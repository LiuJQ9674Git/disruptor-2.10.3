 ...
 *
 *<p>
 *VarHandle是对变量或参数化定义的变量族的动态强类型引用，包括静态字段、非静态字段、
 *数组元素或堆外数据结构的组件。在各种访问模式下支持对这些变量的访问，
 *包括普通读/写访问、易失性读/写存取以及比较和设置。
 *
 * <p>VarHandles are immutable and have no visible state.  VarHandles cannot be
 * subclassed by the user.
 *<p>
 *VarHandles是不可变的，并且没有可见的状态。用户不能对VarHandles进行子类化。VarHandle具有：
 * <p>A VarHandle has:
 * <ul>
 ...
 *
 *<p>
 *每个访问模式都与一个访问模式方法相关联，该方法是以访问模式命名的签名多态方法。
 *在VarHandle实例上调用访问模式方法时，调用的初始参数是坐标表达式，
 *它们精确地指示要访问变量的对象。调用的尾随参数表示访问模式的重要值。
 *例如，各种比较和设置或比较和交换访问模式需要变量的期望值和新值的两个尾随参数。
 *
 ...
 * method signature against which the arguments are checked dynamically.
 *
 *<p>
 *访问模式方法调用的参数的arity和类型不会静态检查。
 *相反，每个访问模式方法都指定一个访问模式类型，表示为MethodType的实例，该类型作为一种方法签名，
 *参数将根据该方法签名进行动态检查。
 *
 * Anaccess mode type gives formal parameter types in terms of the coordinate
 ...
 * given in the access mode type.
 *
 *<p>
 *访问模式方法调用的参数的arity和类型不会静态检查。相反，每个访问模式方法都指定一个访问模式类型，
 *表示为MethodType的实例，该类型作为一种方法签名，参数将根据该方法签名进行动态检查。
 *访问模式类型根据VarHandle实例的坐标类型和对访问模式具有重要意义的值的类型来提供形式化的参数类型。
 *访问模式类型还提供返回类型，通常是根据VarHandle实例的变量类型。
 *在VarHandle实例上调用访问模式方法时，调用站点上的符号类型描述符、
 *调用参数的运行时类型以及返回值的运行时型必须与访问模式类型中给定的类型匹配。
 * A runtime exception will be thrown if the match fails.
 *
 ...
 *
 * <p>
 * 例如，访问模式方法compareAndSet指定，如果其接收器是坐标类型为CT1、…CTn和变量类型T，
 * 则其访问模式类型为（CT1 c1，…，CTn cn，T expectedValue，T newValue）布尔值。
 * 假设VarHandle实例可以访问数组元素，并且其坐标类型为String[]和int，而其变量类型为String。
 * 此VarHandle实例上compareAndSet的访问模式类型为
 * （String[]c1，int c2，String expectedValue，String newValue）布尔值。
 * 这样的VarHandle实例可以由数组工厂方法生成，并访问数组元素如下：
 * <pre> {@code
 * String[] sa = ...
 ...
 *<p>
 *访问模式控制原子性和一致性属性。
 *保证Plain纯读（get）和写（set）访问仅针对引用和最多32位的基元值是逐位原子访问，
 *并且对于执行线程以外的线程不施加可观察的排序约束。
 *
 *不透明Opaque操作是按位原子的，并且相对于对同一变量的访问是一致有序的。
 *除了遵守不透明Opaque属性外，获取模式读取及其后续访问在匹配释放模式写入及
 *其先前访问之后进行排序。除了遵守获取和释放属性外，所有易失性操作都是完全有序的。
 *
 ...
 *
 * 读取访问模式，在指定的内存排序效果下获取变量的值。属于该组的相应访问模式方法集由
 * get、getVolatile、getAcquire和getOpaque方法组成。
 *
 * 
 ...
 * The set of corresponding access mode methods belonging to this group
 * consists of the methods
 * 写访问模式，在指定的内存排序效果下设置变量的值。属于此组的相应访问模式方法集由以下方法组成
 * {@link #set set},
 ...
 * consists of the methods
 * 
 * 原子更新访问模式，例如，在指定的内存排序效果下原子比较和设置变量的值。属于此组的相应访问模式方法集由以下方法组成
 * 
 ...
 *
 *生成或查找VarHandle实例的工厂方法记录了支持的一组访问模式，
 *其中还可能包括记录基于变量类型和变量是否为只读的限制。
 *如果不支持访问模式，则相应的访问模式方法将在调用时抛出UnsupportedOperationException。
 *工厂方法应该记录访问模式方法可能引发的任何其他未声明的异常。所有VarHandle实例都支持get访问模式，
 *相应的方法从不抛出UnsupportedOperationException。
 *
 ...
 *
 *<p>
 *如果VarHandle引用只读变量（例如最终字段），则不支持写入、原子更新、数字原子更新和逐位原子更新访问模式，
 *并且相应的方法抛出UnsupportedOperationException。除了get和set之外，读/写访问模式（如果支持）
 *为引用类型和所有基元类型提供原子访问。除非在工厂方法的文档中另有说明，
 *否则访问模式get和set（如果支持）为引用类型和所有基元类型提供原子访问，32位平台上的long和double除外。
 *
 ...
 *
 *访问模式将覆盖在变量的声明位置site指定的任何内存排序效果。
 *例如，使用获取访问模式访问字段的VarHandle将按照其访问模式指定的方式访问该字段，
 *即使该字段被声明为volatile。当执行混合访问时，应格外小心，因为Java内存模型可能会产生令人惊讶的结果。
 *
 ...
 * 
 *除了支持在各种访问模式下访问变量外，还提供了一组静态方法，称为内存围栏方法，用于对内存排序进行细粒度控制。
 *
 ...
 *
 *Java语言规范允许其他线程观察操作，就好像它们是按照与程序源代码中明显不同的顺序执行的一样，并受到限制，
 *例如，由于使用锁、volatile字段或VarHandles而产生的限制。
 *静态方法fullFence、acquireFence、releaseFence、loadLoadFence和storeStoreFence也可以用于施加约束。
 *它们的规范，就像某些访问模式的情况一样，是根据缺乏“重新排序”来表述的——如果没有围栏，
 *可能会出现可观察到的排序效果。访问模式方法和内存围栏方法的规范的更精确的措辞可能伴随着Java语言规范的未来更新。
 *
