 ...
 *如果没有这样的排队线程，则没有可用于删除的元素，poll（）将返回null。
 *对于其他Collection方法（例如contains），SynchronousQueue充当空集合。此队列不允许空元素。
 ...
 * task.
 *
 *同步队列类似于CSP和Ada中使用的集合通道。它们非常适合切换设计，在切换设计中，
 *运行在一个线程中的对象必须与运行在另一个线程上的对象同步，以便将一些信息、事件或任务交给它。
 ...
 *默认情况下，不保证此订购。然而，公平性设置为true的队列会按照FIFO顺序授予线程访问权限。
 *
...
public class SynchronousQueue<E> extends AbstractQueue<E>
     ...
     *
     *阻塞主要使用LockSupport park/unpark来完成，
     *但似乎是下一个完成的节点首先旋转一位（仅在多处理器上）。在非常繁忙的同步队列上，
     *旋转可以显著提高吞吐量。而在不那么繁忙的地方，旋转的量很小，不会引起注意。
     *
     ...
     * accessing the stack.
     *
     *在队列和堆栈中以不同的方式进行清洁。对于队列，当节点被取消时，
     *我们几乎总是可以在O（1）时间内立即删除它（一致性检查的模重试）。
     *但是，如果它可能被固定为当前尾部，它必须等到随后的取消。
     *对于堆栈，我们需要一个潜在的O（n）遍历，
     *以确保我们可以删除节点，但这可以与访问堆栈的其他线程同时运行。
     *
     ...
     * park/unpark.
     *
     * 当许多线程产生和/或消耗数据时，上述步骤提高了吞吐量。
     * 但它们对单输入/单输出的使用没有太大帮助，在这种情况下，
     * 一侧或另一侧总是被暂时阻塞，因此吞吐量主要是线程调度的函数。
     * 这通常不会在有界短自旋等待的情况下得到显著改善。
     * 相反，这两种形式的转移都会尝试Thread.eyield，如果它显然是唯一的等待者。
     * 当有更多的任务作为核心时，这会很好地工作，预计这将是该模式的主要使用上下文。
     * 在其他情况下，服务员可能会帮忙记账，然后阻塞/释放阻塞park/unpark。
     */
