 ...
 * exceptions!
 *
 * 用于执行低级、不安全操作的方法的集合。尽管类和所有方法都是公共的，
 * 但此类的使用是有限的，因为只有受信任的代码才能获得它的实例。
 * 注意：调用方有责任确保在调用此类的方法之前检查参数。
 * 虽然对输入执行一些基本检查，但这些检查是尽力而为的，并且当性能是压倒一切的优先级时，
 * 例如当运行时编译器优化此类的方法时，可能会忽略一些或所有检查（如果有的话）。
 * 因此，调用方不能依赖于检查和相应的异常！
 * 
 * @author John R. Rose
 * @see #getUnsafe
...
public final class Unsafe {
     ...
     *
     * 为调用方提供执行不安全unsafe操作的能力。返回的Unsafe对象应由调用者小心保护，
     * 因为它可以用于读取和写入任意内存地址的数据。决不能将其传递给不受信任的代码。
     ...
     *
     * <p>
     * 从给定的Java变量中获取一个值。更具体地说，在给定偏移量处，
     * 或者（如果o为空）从其数值为给定偏移量的存储器地址获取给定对象o内的字段或数组元素。
     * 
     * <p>
     ...
     * field's class.
     *
     * 偏移量是从某个Java字段的字段上的objectFieldOffset获得的，
     * o引用的对象属于与该字段的类兼容的类。
     * 
     ...
     *
     * 偏移量和对象引用o（空或非空）都是通过staticFieldOffset和staticFieldBase
     *（分别）从一些Java字段的反射字段表示中获得的。
     *
     ...
     *
     * o引用的对象是一个数组，偏移量是形式为B+N*S的整数，其中N是数组的有效索引，
     * B和S是arrayBaseOffset和arrayIndexScale（分别）从数组的类中获得的值。
     * 所引用的值是数组的第N个元素。
     * </ul>
     * <p>
     ...
     * mode.
     *
     * 如果上述情况之一为真，则调用将引用特定的Java变量（字段或数组元素）。
     * 但是，如果该变量实际上不是此方法返回的类型，则结果是未定义的。
     * 该方法通过两个参数引用一个变量，因此它为Java变量提供了（实际上）双寄存器寻址模式。
     * 当对象引用为null时，此方法使用其偏移量作为绝对地址。
     * 这在操作上类似于getInt（long）等方法，
     * 后者为非Java变量提供（实际上）单寄存器寻址模式。
     * 然而，因为Java变量在内存中的布局可能与非Java变量不同，
     * 所以程序员不应该认为这两种寻址模式是等效的。
     ...
     *
     * 将值存储到给定的Java变量中。前两个参数的解释与getInt（Object，long）完全相同，
     * 以引用特定的Java变量（字段或数组元素）。
     * 给定的值存储在该变量中。
     * 变量的类型必须与方法参数x的类型相同。
     * 
     * parameter {@code x}.
