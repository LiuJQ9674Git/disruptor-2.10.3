 ...
 *虽然此队列在逻辑上是无限制的，但由于资源耗尽（导致OutOfMemoryError），
 *尝试添加可能会失败。此类不允许使用null元素。
 ...
 *方法迭代器iterator中提供的迭代器和方法Spliterator中的spliterater不能保证
 *以任何特定顺序遍历PriorityBlockingQueue的元素。如果您需要有序遍历，
 *请考虑使用Arrays.sort（pq.toArray（））。此
 *外，方法drainTo可以用于按优先级顺序删除部分或全部元素，并将它们放置在另一个集合中。
 *
...
public class PriorityBlockingQueue<E> extends AbstractQueue<E>
     ...
     *
     * 该实现使用基于数组的binary堆，公共操作由单个锁保护。
     * 然而，调整大小期间的分配使用一个简单的自旋锁（仅在不持有主锁时使用），
     * 以便允许take与分配同时操作。这避免了等待消费者的一再推迟和随之而来的要素积累。
     * 在分配过程中需要退出锁，这使得不可能像在该类的早期版本中那样简单地将委托的
     * java.util.PriorityQueue操作封装在锁中。
     * 为了保持互操作性，在序列化过程中仍然使用普通的PriorityQueue，
     * 这以暂时加倍开销为代价来保持兼容性。
     */
     ...
     *
     * 优先级队列表示为平衡binary堆：队列[n]的两个子级是队列[2*n+1]和队列[2*（n+1）]。
     * 如果comparator为null，则优先级队列由comparator或元素的自然顺序排序：
     * 对于堆中的每个节点n和n的每个子代d，n<=d。假设队列为非空，则值最低的元素在queue[0]中。
     */
